...
allow for the runtime simulation of arbitrary precision \YC{up to binary64, so
...
error propagation \YC{amplification} given its ill-defined nature, especially
in multi-resolution pyramids where coarse-stage inaccuracies amplify in finer
stages. A recent studiy~\cite{Mirhakimi2025-qb} on the ANTs \YC{linear?}
...
convergence ($1\times10^{-6}$\YC{I would remove the 1x}) or a maximum iteration
count is reached.
...
optimizer~\cite{Kingma2014-uf} with a learning rate of $1\times10^{-4}$
\YC{same comment for 1x} and a batch size of 4. The loss function combined mean
...
Figure~\ref{fig:pmin-std-bounds} illustrates the required precision per
iteration, estimated via our theoretical model, and the virtual precision set
by the VPREC backend. We report the average across all subjects ($N=50$) with
standard deviation bounds. In the SyN stage, our model indicates that FP24
remains sufficient for convergence. Conversely, during rigid and affine stages,
FP32 suffices for most iterations at level 0. However, as resolution increases
and smoothing decreases, the required precision escalates, eventually exceeding
FP32. Across all multi-resolution levels, the predicted requirement always
breaches the FP32 threshold, though it occasionally lowers back below it before
the registration level finishes.
\begin{figure*}[htb!]
    \centering
...
Figure~\ref{fig:rel-diff} displays the relative accuracy difference between our
DMP method and the FP64 baseline at the final iteration, where lower values
indicate better performance for our method. We report individual subject
differences, along with the mean and quantiles. Overall, the mean relative
difference stays below $1\times10^{-4}$, with the vast majority of values
($591/600)$ falling below $1\times10^{-3}$. Notably, our DMP model frequently
outperforms the baseline ($267/600$), an effect most pronounced in the affine
stage at level 0.
\begin{figure*}[htb!]
    \centering
