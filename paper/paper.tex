...
Gradient-based optimization is the engine behind many medical image analysis tasks, ranging from classical image registration to the training of deep learning (DL) models. However, these optimization processes are computationally demanding. In particular, 3D deformable registration---essential for comparing images across subjects or time points---remains a significant bottleneck for runtime and memory~\cite{Dugre2025-fs}. As datasets grow larger and models more complex, reducing these computational costs is critical for scalability in clinical and research settings.

Mixed precision computing offers a solution by strategically combining high- and low-precision arithmetic to reduce runtime and memory footprints while maintaining accuracy. While widely adopted in DL---thanks to automatic mixed precision (AMP) in PyTorch~\cite{Paszke2019-vk}---its application in classical medical imaging remains limited. Moreover, existing approaches for identifying optimal precision often rely on expensive static analysis or fixed heuristics, failing to adapt to the dynamic numerical stability requirements of specific optimization processes.

Recent advances have sought to accelerate pipelines through hardware-specific implementations, such as GPU-ported registration (FireANTs~\cite{Jena2024-ud}) or DL-based substitutes (VoxelMorph~\cite{Balakrishnan2019-rx}, FastSurfer~\cite{Henschel2022-wa}). While effective, these hardware accelerations are not mutually exclusive with precision optimization. In fact, modern GPUs are designed to maximize throughput specifically for lower-precision formats (e.g., FP16, BF16). Therefore, a theoretical approach to dynamic precision control could benefit both classical CPU-based pipelines and modern GPU-accelerated DL models alike.

In this work, we introduce DynoMP, a dynamic mixed precision method that adjusts bit-width at runtime based on the theoretical convergence stability of the optimization. Unlike standard AMP, which relies on fixed heuristics, DynoMP estimates the signal-to-noise ratio of the gradient to determine the minimum required precision per iteration. We evaluate our method on diverse gradient-based tasks: classical ANTs registration~\cite{Avants2020-xx}, standard image classification on MNIST~\cite{Deng2012-hh}, and DL-based registration using VoxelMorph~\cite{Balakrishnan2019-rx}. We demonstrate that DynoMP maintains the accuracy of high-precision baselines while unlocking the efficiency potential of lower-precision formats. Our contributions include: (1) a theoretical framework for estimating minimum precision requirements during optimization, (2) a practical implementation of dynamic precision control for both classical and DL pipelines, and (3) empirical validation across multiple tasks and datasets, showcasing the potential for resource-efficient medical image computing.
